...
<?php if ( have_comments() ) : ?>
	...
	<p class="title"><?php comments_number(__('No Comments','wpzoom'), __('One Comment','wpzoom'), __('% Comments','wpzoom') );?></p>
	...
	<?php else : // or, if we don't have comments:
		...
		if ( ! comments_open() ) :
	?>
		<!--<p class="title"><?php _e( 'Comments are closed.', 'wpzoom' ); ?></p>-->
...
$custom_comment_form = array( 'fields' => apply_filters( 'comment_form_default_fields', array(
    'author' => '<div class="form_fields">
	<p class="comment-form-author">' .
			'<label for="author">' . __( 'Your Name' , 'wpzoom' ) . 
			( $req ? ' <span class="required_lab">*</span>' : '' ) . '</label> ' .
 			'<input id="author" name="author" type="text" value="' .
			esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' class="required" />' .
 			'</p>',
    'email'  => '<p class="comment-form-email">' .
			'<label for="email">' . __( 'Your Email' , 'wpzoom' ) . 
			( $req ? ' <span class="required_lab">*</span>' : '' ) . '</label> ' .
